...
export default function BackgroundCyber() {
  ...
  useEffect(() => {
    ...
    const drawSecurityIcon = (x: number, y: number, type: string, size: number, alpha: number) => {
      ...
      ctx.strokeStyle = type === "warning" ? "rgba(239, 68, 68, 0.8)" : "rgba(255, 255, 255, 0.8)"
      ctx.fillStyle = type === "warning" ? "rgba(239, 68, 68, 0.3)" : "rgba(255, 255, 255, 0.3)"
      ctx.lineWidth = 1.5
    ...
    const draw = () => {
      ...
      if (!prefersReduced) {
        ctx.fillStyle = "rgba(255, 255, 255, 0.8)"  // White matrix chars
        ...
        for (let i = 0; i < drops.length; i++) {
          ...
          ctx.fillStyle = "rgba(255, 255, 255, 0.3)"  // Faded white trail
